...
#
# A neural network - specifically a multi-layer perceptron (MLP)
# algorithm - is used to predict whether a passenger survives or not.
...
# need to normalise data of each feature to improve
# neural network performnace ( x - mean(x) )/std(x)
...
# choose how many hidden layers and neurons
nLayers = (3,)

# build neural network
neuralNetwork = MLPClassifier( hidden_layer_sizes=nLayers, max_iter= 10000, solver='lbfgs', alpha=1e-6 )
...
yTest = neuralNetwork.predict( xTest )
...
for id, value in enumerate( yTest ) :
    print value
